...
GIL существует потому, что сам интерпретатор не может работать параллельно. В принципе можно было бы
его доработать для этого. Но, насколько я понимаю, этого не делают потому, что это неизбежно
снизит скорость работы однопоточного варианта.
...
* Без необходимости раскрашивать код этих функций, как описано для последующих решений.
...
А также всю возникающую из-за этого неочевидность (в коде ничего не видно, он выглядит синхронным) 
и сложность отладки (попробуй понять, асинхронно выполняется твой код, или нет).

#### Свойства
Можно попробовать применить для кода, который не предполагал асинхронность.
Хотя результат и не гарантирован, причем проблемы не просто будет обнаружить.
...
#### Свойства
Не рационально использовать с большим объемом унаследованого кода. Для такой ситуации стоит 
попробовать варианты с `monkey patching` - возможно, все получится.

Также не стоит использовать `asyncio`, если у используемых вами библиотек ввода/вывода нет 
...
то рациональнее для начала попробовать сделать `monkey patching`, чем тратить много усилий на
создание `asyncio` версии своими силами. 

Впрочем, вместе с тем можно далее в новом коде использовать уже `asyncio` (тем или иным образом решив
вопрос использования единого `цикла выполнени` для этих двух библиотек).
Имеющий, на мой взгляд, существенное преимущество в своей явной природе, упрощающей дальнейшее 
