...
Без использования в реализации функции `super()` Python найдет по списку MRO
первый метод и вызовет его. Даже если первый родитель `Platypus`
не переопределяет этот метод, а второй переопределяет, мы не провалимся до
`Animal` а вызовем метод второго родителя, логичное и ожидаемое поведение.
...
вызов методов всех классов из списка MRO.

`super()` это не класс-родитель, это объект, позволяющий найти следующий по списку 
MRO класс с указанным методом.
...
Код ниже иллюстрирует происходящее. Я добавил аргументы, чтобы вызвать ошибку и 
явно  показать что,  например, из `Mammal.__init__` вызывается следующий родитель 
